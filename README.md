...
> **v1.2.3** — Review results and fixes now render directly in the chat transcript (via `pi.appendEntry` entry renderers) so you can read them inline, yet they stay invisible to the LLM on subsequent prompts. Review scope defaults to the **last prompt only**.
